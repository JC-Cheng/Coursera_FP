...
object Huffman {
   ...
   */
  def until[U](cond: List[U] => Boolean, task: List[U] => List[U])(Input: List[U]): List[U] = {
    if (cond(Input)) Input
    else until(cond, task)(task(Input))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeIter(BitSeq: List[Bit], subtree: CodeTree, Acc: List[Char]): List[Char] = subtree match {
      case Fork(l, r, _, _) => {
        if (BitSeq.head == 0) decodeIter(BitSeq.tail, l, Acc)
        else if (BitSeq.head == 1) decodeIter(BitSeq.tail, r, Acc)
        else throw new java.util.NoSuchElementException("Encoding should contain only 0 and 1")
      }
      case Leaf(c, _) => if (BitSeq.isEmpty) Acc ::: List(c) else decodeIter(BitSeq, tree, Acc ::: List(c))
    }

    decodeIter(bits, tree, List())
  }
  ...
  // result: List(h, u, f, f, m, a, n, e, s, t, c, o, o, l)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeIter(CharSeq: List[Char], subtree: CodeTree, Acc: List[Bit]): List[Bit] = subtree match {
      case Fork(l, r, _, _) => {
        if (CharSeq.isEmpty) Acc
        else if (chars(l).contains(CharSeq.head)) encodeIter(CharSeq, l, Acc:::List(0))
        else if (chars(r).contains(CharSeq.head)) encodeIter(CharSeq, r, Acc:::List(1))
        else throw new java.util.NoSuchElementException
      }
      case Leaf(c, _) => {
        assert(c == CharSeq.head)
        encodeIter(CharSeq.tail, tree, Acc)
      }
    }
    encodeIter(text, tree, List())
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    // assuming a and b to be non-conflict
    def prepend_bit(bit: Bit)(codeTable: CodeTable): CodeTable = codeTable.map((x: (Char, List[Bit])) => (x._1, bit::x._2))
    prepend_bit(0)(a) ::: prepend_bit(1)(b)
  }
